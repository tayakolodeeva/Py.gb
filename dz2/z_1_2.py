...
class Bank:
    ...
    def _in(self, cash: int, tax: int) -> tuple[int, int] | None:
        if cash % self._MIN == 0:
            ...
            self._OPERATION += 1
            self._OPERATIONS[f'+ {cash + tax}'] = 'Выполнено пополнение средств'
    ...
    def _out(self, cash: int, commission: int, tax: int) -> tuple[int, int] | None:
        if cash % self._MIN == 0 and self._BALANCE > 0 and self._BALANCE - (cash + commission + tax) >= 0:
            ...
            self._OPERATION += 1
            self._OPERATIONS[f'- {cash + commission + tax}'] = 'Выполнено снятие наличных'
            return self._BALANCE, self._OPERATION
        else:
            return None
    ...
    def _check_tax(self, cash: int) -> int:
        if cash >=self._MAX:
            print(f'\nВыполнено снятие налога в размере {cash * self._TAX}')
            return cash * self._TAX
        else:
            return 0

    def _exit(self):
        return "Операции завершены"

    def add_bonus(self):
            self._BALANCE += self._BALANCE * self._BONUS
            return f'Начислен бонус . ' \
                   f'На счет поступили средства: {int(self._BALANCE * self._BONUS)}\n'
